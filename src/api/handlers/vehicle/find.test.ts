...
import { findVehicleHandler } from './find';

describe('findVehicleHandler', () => {
  beforeEach(() => {
    mockDb.mockImplementation(() => Promise.resolve([]));
  });

  test('should return 200 when finding a vehicle', async () => {
    ...
    const result = await findVehicleHandler(ctx);
